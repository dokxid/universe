...
export async function getStoryImageUrl(lab: string) {
    ...
    return process.env.LOCAL_UPLOADER === "true"
        ? `http://localhost:3000/uploads/${lab}/story-img/${filename}`
        : `https://${process.env.NEXT_PUBLIC_S3_BUCKET}.s3.${process.env.NEXT_PUBLIC_AWS_REGION}.amazonaws.com/${lab}/story-img/${filename}`;
}
